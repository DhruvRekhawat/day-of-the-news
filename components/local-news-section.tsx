...
export function LocalNewsSection({ news }: LocalNewsSectionProps) {
  if (!news?.length) return null

  return (
    <section className="mb-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Local News</h2>
        ...
        {news.map((item) => (
          <article key={item.id} className="group cursor-pointer">
            ...
            <Link href={`/article/${item.id}`}>
              <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-2 group-hover:text-blue-600 transition-colors line-clamp-2 cursor-pointer">
                {item.title}
              </h3>
            </Link>
